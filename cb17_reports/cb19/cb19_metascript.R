...
tryCatch({
  source("~/gymnast/R/util.R", chdir = TRUE)
  gymnast_install()
...
source(REPO_PARENT %+% "analysis/cb17_reports/cb19/cb19_helpers.R")
...
complete_file_name <- "CB19_Session_1_OFFICIAL_Oct_31_2019.csv"
partial_file_name <- "CB19_Session_1_OFFICIAL-Responses in Progress_Oct_31_2019.csv"
...
# will become unique.
table(duplicated(names(compl_raw)))
...
neptune_df <- util.find_crypt_paths(list(p = "raw_data/neptune_df.csv")) %>%
  unlist %>%
  read.csv
...
text_entries_path <- RMD_BASE_PATH %+% "cb19/text_entries.R"
...
buff_df <- neptune_df %>%
  ...
  rename(survey_id_2019 = "2019", survey_id_2018 = "2018", survey_id_2017 = "2017_fall")

# Recode from older codes
## Looks like this is no longer an issue, but good to check...

# Save originals for checking
s1_merged$survey_id_raw <- s1_merged$survey_id
s1_merged$cohort_name_raw <- s1_merged$cohort_name

recode_df_all <- neptune_df %>%
  group_by(organization_id, cohort_label) %>%
  summarise(cohort_name = first(cohort_name)) %>%
  dcast(organization_id ~ cohort_label) %>%
  rename(cohort_name_2019 = "2019", cohort_name_2018 = "2018",cohort_name_2017 = "2017_fall") %>% 
  merge(buff_df, by = "organization_id", all = TRUE)

# 2017 codes
recode_df <- recode_df_all

recode_df$keep <-  recode_df[,c("survey_id_2019","survey_id_2017")] %>% apply(.,1 , function (x) all(!is.na(x)))
...
s1_merged$survey_id <- s1_merged$survey_id %>%
  util.recode(recode_df$survey_id_2017, recode_df$survey_id_2019)

s1_merged$cohort_name <- s1_merged$cohort_name %>%
  util.recode(recode_df$cohort_name_2017, recode_df$cohort_name_2019)


# 2018 codes
recode_df <- recode_df_all

recode_df$keep <-  recode_df[,c("survey_id_2019","survey_id_2018")] %>% apply(.,1 , function (x) all(!is.na(x)))

recode_df <- recode_df %>% dplyr::filter(keep)

#recode_df[recode_df$survey_id_2019 %in% s1_merged$survey_id, ] %>% View

s1_merged$survey_id <- s1_merged$survey_id %>%
  util.recode(recode_df$survey_id_2018, recode_df$survey_id_2019)

s1_merged$cohort_name <- s1_merged$cohort_name %>%
  util.recode(recode_df$cohort_name_2018, recode_df$cohort_name_2019)
...
select_vect <- (s1_merged$survey_id %>% unique) %in% (neptune_df %>% dplyr::filter(cohort_label == "2019") %>% select(survey_id) %>% unlist)
...
survey_ids_2019 <- neptune_df %>% dplyr::filter(cohort_label == "2019") %>% select(survey_id) %>% unlist %>% unname
cohort_names_2019 <- neptune_df %>% dplyr::filter(cohort_label == "2019") %>% select(cohort_name) %>% unlist %>% unname

dim(s1_merged)
s1_merged <- s1_merged %>% dplyr::filter(
  (survey_id %in% survey_ids_2019)
)
dim(s1_merged) # 18 records are gone
...
temp1 <- (s1_rn$school_name) %>% unique %>% sort; temp1
(s1_rn$school_name_full) %>% unique %>% sort
temp2 <- (neptune_df$school_name_nept) %>% unique %>% sort; temp2
table(temp1 %in% temp2) # 50/53 schools in neptune

# No school name errors - One missing School name ""
old_vals <- c(
  "Shenandoah University - College of Arts and Sciences"
)
new_vals <- c(
...
s1_rn$cb19_race[cond_vect] <- s1_rn$t1__demog__race_1_to_18[cond_vect] %>%
  util.recode(
...
# != NA, we override cb19_race with values
cond_vect <- s1_rn$t1__demog__race_count > 1 & !is.na(s1_rn$t1__demog__identifying_race)
s1_rn$cb19_race[cond_vect] <- s1_rn$t1__demog__identifying_race[cond_vect] %>%
  util.recode(
...
# t1__demog__identifying_race == NA, cb19_race <- Multiracial
# overriding prior values
cond_vect <- s1_rn$t1__demog__race_count > 1 & is.na(s1_rn$t1__demog__identifying_race)
s1_rn$cb19_race[cond_vect] <- "Multiracial"
s1_rn$report_race <- s1_rn$cb19_race
...
# recode gender values (there are differences from cb19)
...
s1_rn$disadv_race.1 <- s1_rn$cb19_race %in%  criteria_vect
s1_rn$disadv_race.1[is.na(s1_rn$cb19_race)] <- NA
...
s1_rn$disadv_race.2 <- race_hits &
  (s1_rn$cb19_race %in%  c("Multiracial"))
...
s1_rn$white.1 <- s1_rn$cb19_race %in%  criteria_vect
s1_rn$white.1[is.na(s1_rn$cb19_race)] <- NA

criteria_vect <- c(14,15,16)
race_hits <- compute_multiple_races(s1_rn$t1__demog__race_1_to_18, criteria_vect)
s1_rn$white.2 <-  race_hits & (s1_rn$cb19_race %in%  c("Multiracial"))
...
s1_rc <- s1_rn %>%
  ...
  mark_qualtrics_dups(id_vars = "participant_id") # Fixed function to ensure seconds
...
progress_vars <- c("Q1.2","Q5.1", "t1__demog__tech_difficulty") # 2018 & 2019
...
report_sites_df <- d_f[c("survey_id", "school_name", "school_name_full")] %>%
  ...
  # remove duplicated survey_ids - none
  filter(!school_name_full_qlt %in% c("Shenandoah University - College of Arts and Sciences")) %>%
...
# What percent at each school?
sort(report_sites_df$sample_size / report_sites_df$expected_n)

# # rename survey ids: # None required
# drop_survey_ids <- c("Survey_xxxxxxxx")
# keep_survey_ids <- c("Survey_xxxxxxxx")
# 
...
# drop schools for which we have no neptune records
...
report_sites_df_orig <- report_sites_df # Based on error reporting below.
error_cohorts <- c("ProjectCohort_2fd6xDuA",
                   "ProjectCohort_0SqxKrla",
                   "ProjectCohort_uPlxjpfD"
                   )
if(REMOVE_ERROR_COHORTS){
  report_sites_df <- report_sites_df %>% dplyr::filter(!cohort_name %in% error_cohorts)
}

# Remove perts cohort
report_sites_df <- report_sites_df %>% dplyr::filter(!cohort_name %in% "ProjectCohort_u9ILZoJ6")
...
for(i in 1:nrow(report_sites_df)){
  ...
  rmd_path <- RMD_BASE_PATH %+% "cb19/cb19_report.Rmd"
  report_name <- org_cohort_name %+% ".html" %>% gsub(" ", "_", .)
  report_path   <- RMD_BASE_PATH %+% "cb19/reports/" %+% report_name
...
for (i in 1:nrow(log_df)){
  f_name =  RMD_BASE_PATH %+% 'cb19/reports/'  %+% log_df$cohort_name[i]  %+%  '.html'
  if(file.exists(f_name)) {
    log_df$html_saved[i] <- TRUE
  }
  f_name =  RMD_BASE_PATH %+% 'cb19/reports/'  %+% log_df$cohort_name[i]  %+%  '.pdf'
  if(file.exists(f_name)) {
...
write.csv(log_df, RMD_BASE_PATH %+% 'cb19/log_df.csv', row.names = F)
write.csv(appendix_tbl_full, RMD_BASE_PATH %+% 'cb19/appendix_tbl.csv', row.names = F)


sum(log_df$sample_size)
