...
if(!exists("org_name")){
  org_name <- report_sites_df$school_name_qlt[1]
}

if(!exists("org_name_full")){
  org_name_full <- report_sites_df$school_name_full_qlt[1]
}
...
extra_vars <- c("t1__dv__enthusiasm3__fun",
                "t1__dv__enthusiasm2__enjoy",
                "t1__dv__enthusiasm1__excited",
                "t1__dv__difficulties_pos",
                "t1__dv__diff2__experience_at_first",
                "t1__dv__diff1__transition",
                "y1_belonging") # these are used for the dynamic text


vars_to_include <-c("participant_id", present_subset_groups, present_metrics, 
                    progress_vars, report_vars, extra_vars) %>% unique

# filter data to desired org_name and relevant columns
d <- d_f[
    d_f$survey_id %in% survey_id, vars_to_include]

school_name_full <- d_f$school_name_full[d_f$survey_id %in% survey_id] %>% unique
if (length(school_name_full) > 1) {stop("More than one unique value for school_name_full")}

school_name <- d_f$school_name[d_f$survey_id %in% survey_id] %>% unique
if (length(school_name) > 1) {stop("More than one unique value for school_name")}
...
buff_df <- d[,c("disadv_status",progress_vars)]
...
table_df$stage_0 <- ifelse(table_df[[progress_vars[1]]] == 1, 1,0)
table_df$stage_0[is.na(table_df$stage_0)] <- 0
table_df$stage_1 <- ifelse(table_df[[progress_vars[2]]] == 1, 1,0)
table_df$stage_1[is.na(table_df$stage_1)] <- 0

table_df$stage_2 <- ifelse(!is.na(table_df[[progress_vars[3]]]), 1,0)

table_df[[progress_vars[1]]]<- NULL
table_df[[progress_vars[2]]] <- NULL
table_df[[progress_vars[3]]] <- NULL
...
#reorder columns
if (all(c("Stage or Module", "All Students","Advantaged", "Disadvantaged") %in% names(progress_tbl))) {
...
log_df$adv_n[log_df$survey_id == survey_id] <- (buff_df$disadv_status == "Advantaged") %>% sum(., na.rm = T)
log_df$disadv_n[log_df$survey_id == survey_id] <- (buff_df$disadv_status == "Disadvantaged") %>% sum(., na.rm = T)
log_df$total_n[log_df$survey_id == survey_id] <- nrow(buff_df)
log_df$cohort_name[log_df$survey_id == survey_id] <- org_cohort_name
```
  ...
  for (variable in items_cp$variables) {
    ...
    for( demog_var in names(demogr_vars)) {
      new_tbl <- buff_df %>%
        ...
        as.data.frame()
      # remove missing values, since they might have freq bellow min cell size and prevent category
      # from being displayed
      # log_vect <- new_tbl[,demogr_vars[[demog_var]]] %>% is.na()
      # new_tbl <- new_tbl[!log_vect,]
    ...
    appendix_tbl$survey_id <- survey_id
  }
