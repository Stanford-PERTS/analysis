# In this script we will k-anonymize the EP 2017-2018 data
...
# input files:
## 1 - EP metascript processed data unimputed - comes from a crypt folder
## helper_functions.csv - comes from the k_anonymization folder in analysis

# output files:
## 2 - EP 2017-2018 k-anon data.csv
## token_map.csv
## basic_log_df.csv
## log_df.csv
...
INPUT_FILE_NAME <-  "1 - EP 2017-2018 metascript processed data unimputed.csv"
OUTPUT_FILE_NAME <- "2 - EP 2017-2018 k-anon data.csv"
...
# fix gender
input_df$gender <- util.recode(input_df$gender_1,
...
input_df <- filter(input_df,
            !is.na(student_id),
            !is.na(class_id),
            !is.na(teacher_id),
            !is.na(team_id))

# Remove rows where our internal “testing” variable is not NA
input_df <- filter(input_df, is.na(testing))

# Remove rows where students say that they are rushing or not comfortable answering honestly
input_df <- filter(input_df,
            !honest_comfort %in% 2,
            !rushing %in% c(4, 5))

# only keep one answer per week_start
input_df <- input_df %>%
  arrange(student_id, week_start) %>%
  group_by(student_id, week_start) %>%
  filter(ResponseID %in% first(ResponseID))

# impute ba_gpa_1
input_df <- input_df %>%
  arrange(student_id, week_start) %>%
  group_by(student_id) %>%
  mutate(ba_gpa_1 = first(ba_gpa_1))
...
tree_specs_url <- "https://docs.google.com/spreadsheets/d/e/2PACX-1vTq1gMoN1pMvCDYOhsn7ieXIevw-_n32r_z3DvjSmddiH6dquzyihQ-txIXcDpO8MSG5ikalvRVECbk/pub?gid=2008263081&single=true&output=csv"
...
# save
OUTPUT_PATH <- gsub(INPUT_FILE_NAME, OUTPUT_FILE_NAME, input_path)
write.csv(
  rbound_df,
  OUTPUT_PATH,
  row.names=FALSE
