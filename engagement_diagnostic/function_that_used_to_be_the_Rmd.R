function_that_used_to_be_the_Rmd <- function(
  SUBSET_TYPES,
  REPORT_DATE,
  TEAM_ONLY,
  ...
  for(driver in present_drivers){
    ...
    if(n_weeks > 1){
      ...
      # I will convert the week start to time variable and the library "scales" will handle the spacing
  ...
  create_sublist <- function(driver_name, driver_abreviation) {
    list(
      ...
      open_response_q_ids =
        paste(
          grep(driver_abreviation, all_or_question_ids, value = TRUE),
          collapse = '", "'
        )
    )
  }
  ...
  learning_conditions <-
    lapply(
      names(drivers),
      function(x) create_sublist(x,drivers[[x]])
    ) %>% setNames(names(drivers))
  ...
  output_list <- list(
    ...
    participation_table = participation_table_df,
    #  gsub(
    #  "\n",
    #  "",
    ...
    learning_condition = learning_conditions
  )
  output_list <- c(output_list, open_responses_list)
  return(toJSON(output_list, pretty=TRUE, auto_unbox = TRUE))
}