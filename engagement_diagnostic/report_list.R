...
create_report_list <- function(){
  ...
  if (ANONYMOUS) {
    ...
    if(length(participation_table_df) == 3) {
      names(participation_table_df) <- c("Week of", "Dream Team", "Mr. Smith Math 1")
    }
    if(length(participation_table_df) == 2) {
      names(participation_table_df) <- c("Week of", "Dream Team")
    }
  ...
  min_weekly_users <- agg_metrics_small[agg_metrics_small$reporting_unit_type %in% "class_id" &
                                          agg_metrics_small$subset_type %in% "All Students", "n"] %>% min(na.rm = T)
  if (min_weekly_users < min_cell) {
    TEAM_ONLY = TRUE
    class_title_full <- ""
  ...
  large_rows <- agg_metrics_small %>%
    group_by(
      reporting_unit_name, week_start, subset_type, metric) %>%
    summarise(n_levels = n()) %>%
    filter(subset_type != "All Students") %>%
  ...
  small_subsets <- small_rows[,c("reporting_unit_name", "week_start","subset_type","metric")] %>%
    unique() %>%
    mutate(delete_small_subset = TRUE)
  ...
  # delete values from all rows matching any subset feature/metric/reporting_unit_name/week start
  ...
  agg_metrics_small <- merge(
    agg_metrics_small,
    small_subsets,
    by = c("reporting_unit_name", "week_start","subset_type","metric"),
    all = TRUE
  ) %>%
  ...
  # First, trim the participation table from the earliest date (the "top") down to the first date
  # that has any data. (The expand.grid process in the metascript introduces extra blank
  # rows from before teams ever started, and these need to be removed.)
  # Assume that the 1st column of the table is the week date, and the rest are response rates,
  ...
  if(!names(participation_table_df)[1] %in% "Week of"){
    stop("Rendering participation table failed because column 1 of participation_table_df was not named 'Week of'.")
  }
  # Sanity-check the ordering of rows in the participation table:
  if(any(participation_table_df$week_order != 1:nrow(participation_table_df))){
    stop("Rendering participation table failed because participation_table_df rows don't seem to be in order.")
  }
  # rows are ordered fine, so you can cut "week_start"
  participation_table_df$week_order <- NULL


  # cut upper rows of the table if there are any zero-value rows
  participation_table_df_vals_only <- participation_table_df[, 2:ncol(participation_table_df)] %>% as.data.frame()
  all_zero_responses <- function(vec) { all(vec %in% "0 (0%)") }
  zero_participation_rows <- apply(participation_table_df_vals_only, 1, all_zero_responses)
  first_nonzero_participation_row_index <- which(!zero_participation_rows)[1]
  participation_table_df <-
    participation_table_df[first_nonzero_participation_row_index:nrow(participation_table_df), ]


  # Now, if the table is still too long to display, shorten it to the right max length backwards from today
  ...
  for(q_id in active_or_question_ids) {
  ...
  triage_columns <- c("driver","reporting_unit_name","pct_good","question_text","week_start")
  ...
  for(driver in present_drivers){
    ...
    n_weeks <- driver_df[driver_df$subset_value %in% focus_populations &
    ...
    if(n_weeks > 1){
      ...
      # if the time chart has too many weeks, we need to use sparse labelling
      # I will convet the week start to time variable and the library "scales"" will handle the spacing
      driver_df_time$week_start_tm <- as.POSIXct(driver_df_time$week_start, origin = "1970-01-01")
      driver_df_time$week_start_dt <- driver_df_time$week_start %>% as.Date()
      # set spacing
      present_weeks <- driver_df_time$dates_measured_phrase  %>% levels
      time_label_spacing <- "1 week"
      if (length(present_weeks) > 7) {
        time_label_spacing <- "2 week"
      }
      if (length(present_weeks) > 14) {
        time_label_spacing <- "3 week"
      }
  ...
  drivers <- list(feedback_for_growth = "fg",
                  teacher_caring = "tc",
                  meaningful_work = "mw")

  create_sublist <- function(driver_name, driver_abreviation) {
    list(
      active = driver_name %in% actually_present_drivers,
      timeline_active = !is.na(timeline_charts[[driver_name]]),
      name = driver_desc[[driver_name]]$name,
      name_sent_case = driver_desc[[driver_name]]$name_sent_case,
      name_title_case = driver_desc[[driver_name]]$name_title_case,
      description = driver_desc[[driver_name]]$description,
      graphic_url = image_paths[[driver_name]],
      timeline_chart = timeline_charts[[driver_name]],
      bar_chart = cross_section_charts[[driver_name]],
      open_response_q_ids = paste(
        grep(driver_abreviation, active_or_question_ids, value = TRUE)
        # @todo: check with Chris how to handle this. The original code uses square brackets, and
        # collapsed string, which I don't fully understand.
      )
    )
  }

  # @todo: check with Chris how to name this list. In the original code it has not names
  learning_condition <- lapply(names(drivers), function(x) create_sublist(x,drivers[[x]]))
  ...
  output_list <- list(
    ...
    report_date = paste0(
      months(date(REPORT_DATE)),
      " ",
      day(date(REPORT_DATE)),
      ", ",
      year(date(REPORT_DATE))
    ),
    ...
    participation_table_html = gsub(
      "\n",
      "",
      util.html_table_data_frame(participation_table_df, print.results = FALSE)
    ),
    script_str = open_responses_list,
    learning_condition = learning_condition
  )

  # @todo: check with Chri how to export this list?
  toJSON(output_list, pretty=TRUE)
}