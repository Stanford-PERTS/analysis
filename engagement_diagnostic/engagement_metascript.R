...
REPORT_DATE <- "2018-06-11" # this is the Monday when the reports are due
...
if(RESTRICT_DATA_FOR_TESTING) {
  ...
  # data <- data[data$team_name %in% c("FuelEd Alum"), ]
...
students_missing_too_long <- data_cat_trim_early_non_data %>%
  ...
  dplyr::filter(missing_too_long == TRUE)
...
for(i in 1:nrow(team_study_class_df)) {
    ...
    if(PDF){
        ...
        error = function(e){
          paste0("Error during PDF conversion.", e) %>% message
        }
        )
...
basic_log <- basic_log %>% merge(., code_counts, by = "code", all.x = TRUE, all.y = FALSE)
